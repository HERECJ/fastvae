...
class AliasTable:
    ...
    @staticmethod
    def alias_table_construct_list(multinomial_list):
        cnt = multinomial_list.shape[0]
        ret_lst = [None] * cnt
        for idx in range(cnt):
            ret_lst[idx] = AliasTable.alias_table_construct(multinomial_list[idx])
        return ret_lst
        

    @staticmethod
    def sample_from_alias_table_1d(alias_table, rand_prob_arr):
        num = len(alias_table)
        sample_num = rand_prob_arr.shape[0]
        rand_idx_arr = np.random.randint(0, num, sample_num)
        samples = np.zeros(sample_num, dtype=np.int32)
        for i in range(sample_num):
            idx = rand_idx_arr[i]
    ...
    @staticmethod
    def sample_from_alias_table_s(alias_table, rand_prob, rand_idx):
        idx = rand_idx
        prob = rand_prob
        idx = idx if prob <= alias_table[idx][0] else alias_table[idx][1]
        return idx
    ...
    @staticmethod
    # @profile
    def sample_from_alias_table_l(alias_table_l, c_idx, rand_prob_arr, rand_idx_arr):
        sample_num = rand_prob_arr.shape[0]
        ...
        for i in range(sample_num):
            idx = rand_idx_arr[i]
            prob = rand_prob_arr[i]
            table = alias_table_l[c_idx[i]][idx]
            idx = idx if prob <= table[0] else table[1]
            samples[i] = idx    
        return samples
